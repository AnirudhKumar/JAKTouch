...
//  JAKTouch
//
//  Created by Anirudh on 20/09/13.
...
static CGFloat const kTouchViewSize = 20;
static CGFloat const kTouchViewAlpha = 0.9;
static NSTimeInterval const kTouchAnimationduration = 0.25;

static JAKTouch *touchVisualizer;
...
+ (JAKTouch *)sharedTouchVisualizer;
- (void)showTouches:(NSSet *)touches;
@end

@interface JAKTouchView : UIView
@property (nonatomic, strong) UIView *markView;
@property (nonatomic, strong) UIView *attentionView;
@end

@implementation JAKTouchView
...
- (void)setupAttentionView
{
    ...
    self.attentionView.layer.borderColor = [UIColor greenColor].CGColor;
...
{
    [[JAKTouch sharedTouchVisualizer] showTouches:event.allTouches];
    [self swizzled_sendEvent:event];
}

@end

@implementation JAKTouch
+ (JAKTouch *)sharedTouchVisualizer
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        touchVisualizer = [[JAKTouch alloc] init];
    });
...
- (JAKTouchView *)touchViewForTouch:(UITouch *)touch
{
    JAKTouchView *touchView;
    
    touchView = (JAKTouchView *)CFDictionaryGetValue(touchViews, (__bridge const void *)(touch));
    if(touchView == nil)
    {
        touchView = [[JAKTouchView alloc] initWithFrame:CGRectMake(0, 0, kTouchViewSize, kTouchViewSize)];
...
{
    [[JAKTouch sharedTouchVisualizer] hideAllTouchViews];
}
